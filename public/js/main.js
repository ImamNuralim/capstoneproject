      ...
      const surahListHTML = response.data.map(data => {
        ...
        return `<div class="card px-3 bg-light shadow-sm text-dark-emphasis">
          <div class="card-body p-0 pt-2">
            <h5 class="text-dark-emphasis">Surah-${number}</h5>
            ...
            <div class="d-flex justify-content-between align-items-baseline">
              <p class="text-secondary">Ayat: ${numberOfAyahs}</p>
              <button class="btn btn-outline-primary btn-sm" onclick="getSurahNumber(${number})">Baca sekarang</button>
            </div>
          </div>
        </div>`;
      }).join("")
